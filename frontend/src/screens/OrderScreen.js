...
export default function OrderScreen() {
  ...
  function calculateAmount(orderitem) {
    ...
    const price =
      parseFloat(orderitem.actualPrice) * parseFloat(orderitem.quantity);
    const deliveryCost = parseFloat(orderitem.deliverySelect.cost);

    const exchangeRate = 0.921; // 92.1%
    const amountZAR = exchangeRate * (price + deliveryCost);
    const amountNGN = exchangeRate * price - 0.014 * deliveryCost;
  ...
  const paymentRequest = async (orderitem) => {
    ...
    deliverOrderHandler("Received", orderitem._id, orderitem);
